...
class BindingAdapters {
    ...
    class LoadList {
        ...
        companion object {
            ...
            @BindingAdapter("list", "type", "columns")
            @JvmStatic
            fun RecyclerView.loadList(l: List<Any>?, t: Int?, c: Int?) {
                t?.let { type ->
                    l?.let { list ->
                        c?.let { columns ->
                            when (type) {
            ...
            private fun RecyclerView.loadPosts(list: List<Post>, columns: Int) {
                layoutManager = if (columns == 1)
                    LinearLayoutManager(context)
                else
                    StaggeredGridLayoutManager(columns, RecyclerView.VERTICAL)
